...
// Ensure database table has file columns
...
if(isset($_GET['get_messages']) && isset($_GET['agent_id'])) {
    $agent_id_ajax = intval($_GET['agent_id']);
    ...
    while($msg = $messages->fetch_assoc()) {
        ...
        if(!empty($msg['file_path'])) {
            ...
            $admin_path = 'uploads/agent_chats/' . basename($stored_path);
            if(file_exists($admin_path)) {
                $found_web_path = $admin_path;
            }
            elseif(file_exists('../' . $stored_path)) {
                $found_web_path = '../' . $stored_path;
            }
            elseif(file_exists($stored_path)) {
                $found_web_path = $stored_path;
            }
            elseif(!strpos($stored_path, 'uploads/') && file_exists('uploads/agent_chats/' . $stored_path)) {
                $found_web_path = 'uploads/agent_chats/' . $stored_path;
            }
            
            if($found_web_path) {
                $mime = $msg['file_type'];
                if(strpos($mime, 'image/') === 0) {
                    $file_html = '<div class="file-attachment media-item" data-src="' . htmlspecialchars($found_web_path) . '" data-type="image"><img src="' . htmlspecialchars($found_web_path) . '" loading="lazy"></div>';
                } elseif(strpos($mime, 'video/') === 0) {
                    $file_html = '<div class="file-attachment media-item" data-src="' . htmlspecialchars($found_web_path) . '" data-type="video"><video controls src="' . htmlspecialchars($found_web_path) . '"></video></div>';
                } elseif(strpos($mime, 'audio/') === 0) {
                    $file_html = '<div class="file-attachment"><audio controls src="' . htmlspecialchars($found_web_path) . '"></audio></div>';
                }
            } else {
                error_log("File not found for admin: " . $stored_path);
            }
        }
        
        $message_text = nl2br(htmlspecialchars($msg['message']));
        echo '<div class="message ' . ($is_admin ? 'admin-message' : 'agent-message') . '">
...
// ========== AJAX HANDLER FOR SENDING MESSAGES WITH FILES ==========
if(isset($_POST['action']) && $_POST['action'] == 'send_message_with_file') {
    ...
    if(isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        if(in_array($mime, $allowed)) {
            ...
            $filename = 'admin_' . $admin_id . '_agent_' . $agent_id_post . '_' . time() . '_' . rand(1000,9999) . '.' . $ext;
    ...
    if($response['status'] == 'ok') {
        $stmt = $con->prepare("INSERT INTO admin_agent_messages (agent_id, admin_id, sender_type, message, file_path, file_type) VALUES (?, ?, 'admin', ?, ?, ?)");
        $stmt->bind_param("iisss", $agent_id_post, $admin_id, $message, $file_path, $file_type);
        $stmt->execute();
...
// ========== MAIN PAGE ==========
$agent_info = $con->query("SELECT username, fullname FROM agents WHERE id = $agent_id")->fetch_assoc();
if(!$agent_info) {
    die("Agent not found.");
}
$con->query("UPDATE admin_agent_messages SET is_read = 1 WHERE agent_id = $agent_id AND sender_type = 'agent' AND is_read = 0");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Chat with Agent | Admin</title>
    ...
    <style>
        ...
        .chat-container { max-width: 800px; margin: 20px auto; background: white; border-radius: 28px; border: 1px solid #EFE8E2; overflow: hidden; display: flex; flex-direction: column; height: calc(100vh - 180px); }
        ...
        .admin-message .message-bubble { background: #C47A5E; color: white; border-bottom-right-radius: 4px; }
        .agent-message .message-bubble { background: #F0E9E3; color: #2A2826; border-bottom-left-radius: 4px; }
        .message-time { font-size: 0.65rem; margin-top: 4px; text-align: right; color: #7A726C; }
        .admin-message .message-time { color: #E3DCD5; }
...
<div class="container-fluid px-0">
    <div class="row g-0">
        ...
        <div class="col-md-9 col-lg-10">
            <div class="px-4 pt-3">
                <a href="agent-chats.php" class="back-link"><i class="fas fa-arrow-left"></i> Back to conversations</a>
                <div class="chat-container">
                    <div class="chat-header">
                        <h5 class="mb-0">
                            <i class="fas fa-user-tie"></i> 
                            <?php echo htmlspecialchars($agent_info['username']); ?> 
                            (<?php echo htmlspecialchars($agent_info['fullname']); ?>)
                        </h5>
                    </div>
...
<!-- Image Editing Modal (Cropper.js) -->
...
// Store a flag to know if we should auto-scroll after update
var autoScroll = true;
var isAtBottom = true;
...
function loadMessages() {
    ...
    $.get(window.location.pathname, { get_messages: 1, agent_id: agent_id, t: Date.now() }, function(data) {
        // Store current scroll position relative to the top element (the first message)
        var firstMsg = chatDiv.children().first();
        var offsetBefore = firstMsg.length ? firstMsg.offset().top - chatDiv.offset().top : 0;
        
        chatDiv.html(data);
        
        // After new messages are loaded, decide scroll behavior
        if (isAtBottom) {
            // User was near bottom -> scroll to bottom
            chatDiv.scrollTop(chatDiv[0].scrollHeight);
        } else {
            // User was somewhere in the middle -> try to maintain position relative to the first message
            var newFirstMsg = chatDiv.children().first();
            if (newFirstMsg.length && offsetBefore > 0) {
                var newOffset = newFirstMsg.offset().top - chatDiv.offset().top;
                chatDiv.scrollTop(chatDiv.scrollTop() + (newOffset - offsetBefore));
            } else {
                // Fallback: maintain scrollTop difference
